...
// .wrangler/tmp/bundle-HCRQhf/checked-fetch.js
  ...
  async fetch(request, env, ctx) {
    ...
    if (!url.pathname.startsWith("/api/")) {
      if (env.ASSETS) {
        ...
        console.log(`ASSETS.fetch(${request.url}) returned ${response2.status}`);
        if (response2.status === 404) {
          const fallbackReq = new Request(new URL("/", request.url).toString(), request);
          const fallbackRes = await env.ASSETS.fetch(fallbackReq);
          console.log(`Fallback fetch(/) returned ${fallbackRes.status}`);
          return fallbackRes;
        }
        return response2;
      }
...
// .wrangler/tmp/bundle-HCRQhf/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HCRQhf/middleware-loader.entry.ts
